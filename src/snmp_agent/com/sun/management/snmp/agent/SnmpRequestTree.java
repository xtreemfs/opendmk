 ...
 * varbind. 
 * This class holds a hashtable of Handler nodes, with the involved 
...
//	    if (entryoids != null) count += entryoids.size();
	...
	if (meta == null) {
	    if (logger.finestOn())
		logger.finest("registerNode","meta-node is null!!!");
	    return;
	}
	if (varbind == null) {
	    if (logger.finestOn())
		logger.finest("registerNode","varbind is null!!!");
	    return ;
	}
	...
	if (handler == null) {
	    // if (logger.finestOn()) 
            //    logger.finest("registerNode", "adding node for " +
	...
	// else {
	//   if (logger.finestOn()) 
	//	logger.finest("registerNode","found node for " +
	//	      varbind.oid.toString());
	// }
	...
	new ClassLogger(ClassLogger.LOGGER_SNMP,"SnmpRequestTree");
