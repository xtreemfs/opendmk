/*
 * @(#)file      SnmpUsmKeyHandlerImpl.java
...
/**
 * This class allows you to compute key localization and delta generation. It is useful when adding user in USM MIB. An instance of <CODE> SnmpUsmKeyHandler </CODE> is associated to each <CODE> SnmpEngine </CODE> object.
...
public class SnmpUsmKeyHandlerImpl implements SnmpUsmKeyHandler{
	...
	SnmpUsmAlgorithm algo = manager.getAlgorithm(name);
	...
	return getAlgo(algoName).password_to_key(password);
    }
    /**
	...
	return getAlgo(algoName).localizeAuthKey(key, engineId);
    }
	...
	return getAlgo(algoName).localizePrivKey(key, engineId, keysize);
    }			  
	...
	return getAlgo(algoName).calculateAuthDelta(oldKey, newKey, random);
    }
	...
	return getAlgo(algoName).calculatePrivDelta(oldKey, newKey, random, deltaSize);
    }
}
