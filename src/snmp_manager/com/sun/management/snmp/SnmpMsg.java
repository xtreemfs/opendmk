...
public abstract class SnmpMsg implements SnmpDefinitions {
        ...
        try {
            ...
            if (varBindList != null) {
                for (int i = varBindList.length - 1 ; i >= 0 ; i--) {
                    ...
                    if (bind != null) {
                        benc.openSequence() ;
                        encodeVarBindValue(benc, bind.getSnmpValue()) ;
                        benc.putOid(bind.getOid().longValue()) ;
                        benc.closeSequence() ;
                        encodedVarBindCount++ ;
                    }
                }  
    ...
    void encodeVarBindValue(BerEncoder benc, 
	    ...
	    // NPCTE fix for bugId 4692891, esc 537693, MR,  June 2002
            ...
            benc.putUnsignedLong(((SnmpCounter64)v).longValue(), SnmpValue.Counter64Tag) ;
	...
	throws BerException {
		...
		bdec.openSequence() ;
		bind.setOid(new SnmpOid(bdec.fetchOid()), false);
	...
	//                 BerException.
		...
		// NPCTE fix for bugId 4692891, esc 537693, MR,  June 2002
                ...
                result = new SnmpCounter64(UnsignedLong.make(bdec.fetchIntegerAsLong(tag))) ;
                // end of NPCTE fix for bugId 4692891
	    } catch(RuntimeException r) {
		throw new BerException();
