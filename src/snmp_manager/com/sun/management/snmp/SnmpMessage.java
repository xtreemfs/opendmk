 ...
 * implement your own {@link com.sun.management.snmp.SnmpPduFactory SnmpPduFactory} object.
 * <P>
...
public class SnmpMessage extends SnmpMsg implements SnmpDefinitions {
		...
		if(trapPdu.agentAddr != null)
            ...
            default:
                throw new SnmpStatusException("Invalid pdu type " + 
				     String.valueOf(pdupacket.type)) ;
            }
            benc.closeSequence(pdupacket.type) ;
            dataLength = benc.trim() ;
        }
        catch(ArrayIndexOutOfBoundsException x) {
	    final SnmpVarBind[] vbl = pdupacket.varBindList;
	    final int index = (vbl==null || vbl.length==0)?0:(vbl.length-1);
            final SnmpTooBigException tooBig = 
		new SnmpTooBigException(index);
	    try {
		com.sun.jdmk.defaults.Utils.initCause(tooBig,x);
	    } catch (Throwable t) { /* OK: best effort here */ }
	    throw tooBig;
        }	
    }
    /**
	...
	new ClassLogger(ClassLogger.LOGGER_SNMP,"SnmpMessage");
