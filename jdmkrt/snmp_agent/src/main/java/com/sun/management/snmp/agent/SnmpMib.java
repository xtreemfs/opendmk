...
final class LongList {
    ...
    /**
     * Same behavior {@link java.util.List#size()}.
     **/
    ...
    /**
     * Same behavior than {@link java.util.List#toArray(Object[])}.
     * Any access to this method should be protected in a synchronized 
     * block on the LongList object.
     **/
    ...
    /**
     * Same behavior than {@link java.util.List#toArray()}.
     * Any access to this method should be protected in a synchronized 
     * block on the LongList object.
     **/
...
class AcmChecker {
	...
	new ClassLogger(ClassLogger.LOGGER_SNMP,"AcmChecker");
}
 ...
 * plug in your own specific behavior.
 * </p>
 *
 * @since Java DMK 5.1
 */
public abstract class SnmpMib extends SnmpMibAgent implements Serializable {
     ...
     * defaultOid by an application dependent OID string, for instance.
     * </p>
     *
     * <p>
	...
	// For each varbind in the list finds its handling node.
	    ...
	    if (logger.finestOn()) {
		logger.finest("getHandlers",
			      "Processing subrequest varbind #" + (index+1));
	    }
		...
		// Find the handling node for this varbind.
                root.findHandlingNode(var,var.getOid().longValue(false),
				      0,handlers);
		...
		//
		    ...
		    final  SnmpStatusException sn = 
			new SnmpStatusException(status, index + 1);
		     com.sun.jdmk.defaults.Utils.initCause(sn,x);
		...
		if (logger.finestOn()) {
		    logger.finest("getHandlers","failed for varbind #"+
				  (index+1)+": "+x,x); 
		}
		throw x;
	    }
	...
	// follows.
		...
		//ACM loop.
		if (logger.finestOn()) 
		    logger.finest("getGetNextHandlers"," Next Oid of :" + 
				  var.getOid());
		result = new SnmpOid(root.findNextHandlingNode
				     (var,var.getOid().longValue(false),0,
				      0,handlers, checker));
		...
		// if (logger.finestOn()) 
		//    logger.finest("getGetNextHandlers",
		//	  "Couldn't find a handling node for "
		//	  + var.oid.toString());
		...
		if (logger.finestOn()) {
		    logger.finest("getGetNextHandlers","Exception : " + 
				  x.getStatus());
		    logger.finest("getHandlers", 
				  "\tcaused by: " + x);
		}
    ...
    private static final ClassLogger logger = 
	new ClassLogger(ClassLogger.LOGGER_SNMP,"SnmpMib");
