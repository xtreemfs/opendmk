 ...
 * {@link com.sun.management.snmp.agent.SnmpTableEntryFactory} to which it will
 ...
 * whole MIB class extending {@link com.sun.management.snmp.agent.SnmpMib}
 * You should never need to instantiate this class directly.
 * </p>
 *
 * @see com.sun.management.snmp.agent.SnmpMib
 * @see com.sun.management.snmp.agent.SnmpMibEntry
 * @see com.sun.management.snmp.agent.SnmpTableEntryFactory
 * @see com.sun.management.snmp.agent.SnmpTableSupport
 *
 *
 * @since Java DMK 5.1
 */
...
public abstract class SnmpMibTable extends SnmpMibNode 
     ...
     * behavior, should the default behavior not be at your convenience.
     * </p>
     * <p>
     * @param request   The SNMP  subrequest requesting this creation
     ...
     */
    public abstract void createNewEntry(SnmpMibSubRequest request, SnmpOid rowOid,
					int depth) 
     ...
     *      <CODE>get(request,oid,depth+1)</CODE> method. </li>
     * </ul> 
     * <p>
     * <pre>
     ...
     * <CODE>get(SnmpMibSubRequest request, int depth)</CODE> on the generated 
	...
	final boolean         isnew  = request.isNewEntry();
	final SnmpMibSubRequest  r      = request;

	// if the entry does not exists, then registers an error for
	// each varbind involved (nb: should not happen, the error
	// should have been registered earlier)	
	if (isnew) {
	    if (finest) logger.finest("get","unknown instance: "+
				      r.getEntryOid()); 
	    SnmpVarBind     var = null;
	...
	if (finest) logger.finest("get","getting instance: "+oid); 

	// SnmpIndex   index  = buildSnmpIndex(oid.longValue(false), 0);
	// get(request,index,depth+1);
	//
	get(request,oid,depth+1);
    }
     ...
     * <li> call the generated 
     *      <CODE>check(request,oid,depth+1)</CODE> method. </li>
     * </ul> 
     * <p>
     * <pre>
     ...
     * <p> You should not need to override this method in any cases, because
     * it will eventually call 
     * <CODE>check(SnmpMibSubRequest request, int depth)</CODE> on the generated 
     * derivative of <CODE>SnmpMibEntry</CODE>. If you need to implement
     * specific policies for minimizing the accesses made to some remote
     * underlying resources, or if you need to implement some consistency
     * checks between the different values provided in the varbind list,
	...
	final boolean dbg = logger.finestOn();

	if (dbg) logger.finest("check","Calling beginRowAction");

	beginRowAction(request,oid,depth+1,action);

	if (dbg) logger.finest("check","Calling check for " + request.getSize() + 
		       " varbinds.");

	check(request,oid,depth+1);

	if (dbg) logger.finest("check","check finished");
    }
     ...
     * <p> The default implementation of this method is to
     * call the generated 
     * <CODE>set(request,oid,depth+1)</CODE> method.
     * <p>
     * <pre>
     ...
     * <p> You should not need to override this method in any cases, because
     * it will eventually call 
     * <CODE>set(SnmpMibSubRequest request, int depth)</CODE> on the generated 
     * derivative of <CODE>SnmpMibEntry</CODE>. If you need to implement
     * specific policies for minimizing the accesses made to some remote
     * underlying resources, or if you need to implement some consistency
     * checks between the different values provided in the varbind list,
	...
	final boolean dbg = logger.finestOn();
 	if (dbg) logger.finest("set","Entering set.");

	final SnmpOid     oid    = request.getEntryOid();
	final int         action = getRowAction(request,oid,depth+1);

 	if (dbg) logger.finest("set","Calling set for " + request.getSize() + 
		       "varbinds.");

	set(request,oid,depth+1);

	if (dbg) logger.finest("set","Calling endRowAction");

	endRowAction(request,oid,depth+1,action);

	if (dbg) logger.finest("set","RowAction finished");
    }
     ...
     * {@link com.sun.management.snmp.agent.SnmpTableEntryFactory} interface
     ...
     * required, then 
     * {@link com.sun.management.snmp.agent.SnmpMibTable#addEntry(SnmpOid, ObjectName, Object)} should be preferred.
    ...
    public MBeanNotificationInfo[] getNotificationInfo() {
	    ...
	    (types, "com.sun.management.snmp.agent.SnmpTableEntryNotification", 
     ...
     * <p>
     * @param factory The 
     *        {@link com.sun.management.snmp.agent.SnmpTableEntryFactory} through
     ...
     * by {@link com.sun.management.snmp.EnumRowStatus}. These codes correspond
     * to RowStatus codes as defined in RFC 2579, plus the <i>unspecified</i>
     * value which is Java DMK specific.
     * <p>
     *
     * @param request    The sub-request that must be handled by this node.
     *
     * @param rowOid The <CODE>SnmpOid</CODE> identifying the table
     *               row involved in the operation.
     *
     ...
     *
     * @see com.sun.management.snmp.EnumRowStatus
     **/
    protected int getRowAction(SnmpMibSubRequest request, SnmpOid rowOid,
			       int depth) 
	throws SnmpStatusException {
	final boolean     isnew  = request.isNewEntry();
	final SnmpVarBind vb = request.getRowStatusVarBind();
	if (vb == null) {
	...
	try {
	    return mapRowStatus(rowOid, vb, request.getUserData());
	} catch( SnmpStatusException x) {
	    checkRowStatusFail(request, x.getStatus());
	}
     ...
     * {@link com.sun.management.snmp.EnumRowStatus}.
     * These codes correspond to RowStatus codes as defined in RFC 2579, 
     * plus the <i>unspecified</i> value which is Java DMK specific.
     * <p>
    ...
    protected int mapRowStatus(SnmpOid rowOid, SnmpVarBind vbstatus,
			       Object userData) 
	throws SnmpStatusException {
	final SnmpValue rsvalue = vbstatus.getSnmpValue();
     ...
     *        {@link com.sun.management.snmp.EnumRowStatus}. These codes 
     ...
     *            rejected.
     **/
    protected void checkRemoveTableRow(SnmpMibSubRequest request, SnmpOid rowOid,
				       int depth) 
	throws SnmpStatusException {
     ...
     * specific behavior when a remote row deletion is invoked.
     * <p>
     ...
     *
     **/
    protected void removeTableRow(SnmpMibSubRequest request, SnmpOid rowOid,
				  int depth) 
	throws SnmpStatusException {	
     ...
     * @see com.sun.management.snmp.EnumRowStatus
     **/
    protected synchronized void beginRowAction(SnmpMibSubRequest request, 
	...
	case EnumRowStatus.unspecified:
	    if (isnew) {
		if (logger.finestOn()) 
		    logger.finest("beginRowAction","Failed to create row[" + 
				  rowOid + "] : RowStatus = unspecified");
		checkRowStatusFail(request,SnmpStatusException.snmpRspNoAccess);
	    }
	    break;
 	...
 	case EnumRowStatus.createAndWait:
		...
		if (isCreationEnabled()) {
		    if (logger.finestOn()) 
			logger.finest("beginRowAction","Creating row["+rowOid+ 
			      "] : RowStatus = createAndGo | createAndWait");
		    createNewEntry(request,oid,depth);
		} else {
		    if (logger.finestOn()) 
			logger.finest("beginRowAction","Can't create row["+
				      rowOid + "] : RowStatus = " +
				      "createAndGo | createAndWait" +
				      " but creation is disabled");
		    checkRowStatusFail(request,
                       SnmpStatusException.snmpRspNoAccess);
		}
	    } else {
		if (logger.finestOn()) 
		    logger.finest("beginRowAction","Can't create row["+rowOid+ 
			  "] : RowStatus = createAndGo | createAndWait" +
 	...
 	case EnumRowStatus.destroy:
	    if (isnew) {
		if (logger.finestOn()) 
		    logger.finest("beginRowAction",
				  "Warning: can't destroy row[" + 
				  rowOid + "] : RowStatus = destroy" +
				  " but row does not exist");
	    } else if (!isCreationEnabled()) {
		if (logger.finestOn()) 
		    logger.finest("beginRowAction","Can't destroy row[" +
				  rowOid + "] : RowStatus = destroy " +
				  " but creation is disabled");
		checkRowStatusFail(request,SnmpStatusException.snmpRspNoAccess);
	    }
	    checkRemoveTableRow(request,rowOid,depth);
	    break;
	...
	case EnumRowStatus.notInService:
	    ...
	    checkRowStatusChange(request,rowOid,depth,action);
	    break;
	case EnumRowStatus.notReady:
	default:
	    if (logger.finestOn()) 
		logger.finest("beginRowAction",
			      "Invalid RowStatus value for row[" + 
     ...
     * @see com.sun.management.snmp.EnumRowStatus
     **/
    protected void endRowAction(SnmpMibSubRequest request, SnmpOid rowOid,
			       int depth, int rowAction) 
	throws SnmpStatusException {
	final boolean     isnew  = request.isNewEntry();
	final SnmpOid     oid    = rowOid;
	final int         action = rowAction;
	final Object      data   = request.getUserData();
	...
	case EnumRowStatus.unspecified:
 	...
 	case EnumRowStatus.createAndGo:
	    if (logger.finestOn()) 
		logger.finest("endRowAction","Setting RowStatus to `active'" +
 	...
 	case EnumRowStatus.destroy:
	    ...
	    removeTableRow(request,oid,depth);
	    break;
	case EnumRowStatus.active:
	     if (logger.finestOn()) 
		logger.finest("endRowAction",
	...
	default:
		...
		logger.finest("endRowAction",
			      "Invalid RowStatus value for row[" + 
			      rowOid + "] : specified RowStatus = " + action);
	    setRowStatusFail(request,
			  SnmpStatusException.snmpRspInconsistentValue);
	}
	if (value != null) {
	    final SnmpVarBind vb = request.getRowStatusVarBind();
	    if (vb != null) vb.setSnmpValue(value);
	}
    }
     ...
     * java.lang.Object)},
     * until
     * {@link #skipEntryVariable(com.sun.management.snmp.SnmpOid,long,
     ...
     * <CODE>SnmpMibTable</CODE> subclasses generated by <CODE>mibgen</CODE>.
     */
    abstract protected void get(SnmpMibSubRequest request, 
				SnmpOid rowOid, int depth) 
	throws SnmpStatusException;
     ...
     * <CODE>SnmpMibTable</CODE> subclasses generated by <CODE>mibgen</CODE>.
     */
    abstract protected void check(SnmpMibSubRequest request, 
				  SnmpOid rowOid, int depth) 
	throws SnmpStatusException;
     ...
     * <CODE>SnmpMibTable</CODE> subclasses generated by <CODE>mibgen</CODE>.
     */
    abstract protected void set(SnmpMibSubRequest request, 
				SnmpOid rowOid, int depth) 
	throws SnmpStatusException;
    ...
    final static void checkRowStatusFail(SnmpMibSubRequest request, 
					 int errorStatus)
	throws SnmpStatusException {
	final SnmpVarBind statusvb  = request.getRowStatusVarBind();
    ...
    final static void setRowStatusFail(SnmpMibSubRequest request, 
				       int errorStatus)
	throws SnmpStatusException {
	final SnmpVarBind statusvb  = request.getRowStatusVarBind();
	final SnmpStatusException x = new SnmpStatusException(errorStatus);
	...
	throws SnmpStatusException {
		...
		// points to the arc of the querried variable 
		// (columnar object).
		...
		// is empty, so we can let the exception propagate to the 
		// caller.
		//
		
		// We must check whether the variable must be skipped...
		entryoid = getNextOid(data);
		...
		// We build a possibly incomplete and invalid index from the 
		// OID.
    ...
    /**
     * Look for the given oid in the OID table and returns
     * its position.
     *
     * <p>
	...
	new ClassLogger(ClassLogger.LOGGER_SNMP,"SnmpMibTable");
